...
void childevent(pid_t pid, char *event, int sig);
...
void eval(char *cmdline) 
{
	...
	/* initialize mask set and add SIGCHLD to the set */
	Sigemptyset(&mask);
	...
	/* return if no arguments are passed */
	if (argv[0] == NULL)
		return;

	/* execute a new program if cmdline is not shell builtin */
	if (!builtin_cmd(argv)) {
...
void sigchld_handler(int sig) 
{
	...
	/* wait for child process to be terminated or stopped */
	while ((pid = waitpid(-1, &wstatus, WNOHANG | WUNTRACED)) > 0) {
		...
		/* terminated by a signal */
			...
			childevent(pid, "terminated", WTERMSIG(wstatus));
			deletejob(jobs, pid);
		}

		/* stopped by a signal */
		else if (WIFSTOPPED(wstatus)) {
			childevent(pid, "stopped", WSTOPSIG(wstatus));
...
/*
 * childevent - print information about a terminated or stopped child
 */
void childevent(pid_t pid, char *event, int sig)
{
	Sio_puts("Job [");
